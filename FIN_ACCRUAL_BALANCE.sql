CREATE OR REPLACE PACKAGE                             FIN_ACCRUAL_BALANCE AS

   PROCEDURE FIN_ACCRUAL_BALANCE(	inp_str      IN  VARCHAR2,
			out_retCode  OUT NUMBER,
			out_rec      OUT VARCHAR2 );

END FIN_ACCRUAL_BALANCE;
/


CREATE OR REPLACE PACKAGE BODY                                                                                                                                                                  FIN_ACCRUAL_BALANCE AS
...
CURSOR ExtractDataRec (	
  ...
  IS
      ...
      WHERE
       ...
       And  (Eit.Nrml_Interest_Amount_Dr <> 0 or  EIT.NRML_ACCRUED_AMOUNT_DR <> 0 )
...
CURSOR ExtractDataPay (	
  ...
  IS
      ...
      WHERE
         ...
         --and TO_DATE( CAST ( ci_endDate AS VARCHAR(10) ) , 'dd-MM-yyyy' )
         And (Eit.Nrml_Accrued_Amount_Cr <> 0 or  EIT.NRML_INTEREST_AMOUNT_CR <> 0)
         and GAM.SCHM_TYPE LIKE '%' ||UPPER(ci_SchemeType) || '%' 
         and GAM.SOL_ID LIKE '%' ||ci_branchCode || '%' 
         and gam.schm_code LIKE '%' ||UPPER(ci_SchemeCode) || '%' 
         and gam.acct_crncy_code = upper(ci_currency)
