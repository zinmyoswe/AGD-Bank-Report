CREATE OR REPLACE PACKAGE        FIN_ACQ_TRANS_WITHDRAW_LISTING AS

  /* TODO enter package declarations (types, exceptions, methods etc) here */
  PROCEDURE FIN_ACQ_TRANS_WITHDRAW_LISTING(	inp_str      IN  VARCHAR2,
                                            out_retCode  OUT NUMBER,
                                            out_rec      OUT VARCHAR2 );

END FIN_ACQ_TRANS_WITHDRAW_LISTING;


/


CREATE OR REPLACE PACKAGE BODY                                                                                                                                                                                FIN_ACQ_TRANS_WITHDRAW_LISTING AS
/******************************************************************************
 NAME:       FIN_ACQ_TRANS_WITHDRAW_LISTING
 PURPOSE:
...
--------------------------------------------------------------------------------
  ...
  vi_startTime      VARCHAR2(8);               -- Input to procedure
  vi_endTime        VARCHAR2(8);               -- Input to procedure
  vi_exRate         VARCHAR2(10);               -- Input to procedure
--------------------------------------------------------------------------------
-- CURSOR declaration FIN_ACQ_TRANS_WITHDRAW_LISTING CURSOR
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- CURSOR ExtractData
--------------------------------------------------------------------------------
...
IS
  SELECT NVL(D.INSTITUTION_ID,LTRIM(SUBSTR(DTT.MESG,INSTR(DTT.MESG,'BC', 1, 1)-10,8), '2')) AS INSTITUTION_ID
        , SUBSTR(NVL(REGEXP_SUBSTR(D.CARDTYPE,'[^|]+',1,1), ' '),1,10) AS BINCODE, D.CHANNEL_DEVICE_ID AS TERMINAL_ID--SUBSTR(NVL(M.CARD_NUMBER, ' '),1,6)
