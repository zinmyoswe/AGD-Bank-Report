CREATE OR REPLACE PACKAGE FIN_ATM_BILL_TOPUP_LISTING AS 

  /* TODO enter package declarations (types, exceptions, methods etc) here */ 
  PROCEDURE FIN_ATM_BILL_TOPUP_LISTING(	inp_str       IN  VARCHAR2,
                                        out_retCode   OUT NUMBER,
                                        out_rec       OUT VARCHAR2 );

END FIN_ATM_BILL_TOPUP_LISTING;
/


CREATE OR REPLACE PACKAGE BODY FIN_ATM_BILL_TOPUP_LISTING AS
...
IS  
  SELECT  DTT.TRAN_ID AS ENTRY_NO, D.CHANNEL_DEVICE_ID AS ATM_NO, G.FORACID AS ACC_NO
          , ' ' AS TOPUP_REF_NO, DTT.SNO, DTT.TRAN_AMT
          , CASE WHEN TRIM(SUBSTR(MESG, INSTR(MESG, 'M001', 1, 1)+4 , 1)) IN ('1','2') THEN 'GSM' ELSE 'CDMA' END AS TOPUP_TYPE
          , ' ' AS EXPIRE_DATE, DTT.PSTD_DATE AS DATE_PART, TO_CHAR(DTT.PSTD_DATE, 'HH24:MI:SS') AS TIME_PART
  FROM
  (
...
IS  
  ...
  v_TopUpRefNo VARCHAR2(20);
  v_SrNo VARCHAR2(20);  
  v_TranAmt TBAADM.REF_TRN_TBL.TRAN_AMT%type;
  v_TopUpType VARCHAR2(10);
  v_ExpireDate VARCHAR2(10);
  ...
  ------------------------------------------------------------------------------
    ...
    --{
      ...
      INTO v_EntryNo, v_ATMNo, v_AccNo, v_TopUpRefNo, v_SrNo, v_TranAmt, v_TopUpType, v_ExpireDate, v_DatePart, v_TimePart;
------------------------------------------------------------------------------
