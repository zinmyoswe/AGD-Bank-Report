CREATE OR REPLACE PACKAGE                      FIN_AUTO_SAVING_LINK_DEBIT AS 

   PROCEDURE FIN_AUTO_SAVING_LINK_DEBIT(	inp_str      IN  VARCHAR2,
			out_retCode  OUT NUMBER,
			out_rec      OUT VARCHAR2 );

END FIN_AUTO_SAVING_LINK_DEBIT;
 
/


CREATE OR REPLACE PACKAGE BODY                                                                FIN_AUTO_SAVING_LINK_DEBIT AS
...
CURSOR ExtractData (	
  ...
  GAM.FORACID as "Account No" ,
   ...
   CTD_DTD_ACLI_VIEW.TRAN_ID as "Entry Id" , 
   CTD_DTD_ACLI_VIEW.TRAN_AMT as "Amount" 
from 
   TBAADM.GAM GAM , 
   TBAADM.CTD_DTD_ACLI_VIEW CTD_DTD_ACLI_VIEW , 
   TBAADM.PFT PFT 
where
   GAM.SOL_ID = ci_branchCode 
   and GAM.POOL_ID = PFT.POOL_ID 
   and CTD_DTD_ACLI_VIEW.ACID = GAM.ACID 
   and GAM.del_flg = 'N'
  ...
  and CTD_DTD_ACLI_VIEW.PART_TRAN_TYPE  = 'D'
  ...
  PROCEDURE FIN_AUTO_SAVING_LINK_DEBIT(	inp_str      IN  VARCHAR2,
    ...
    v_EntryId tbaadm.CTD_DTD_ACLI_VIEW.TRAN_ID%type;
...
---------------------------------------------------------------------------------------------------------
    ...
    IF ExtractData%ISOPEN THEN
		--{
			FETCH	ExtractData
			INTO	v_AccountNumber, v_AccountName, v_EntryId, v_Amount 
            ;
