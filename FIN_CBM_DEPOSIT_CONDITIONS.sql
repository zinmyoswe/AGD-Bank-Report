CREATE OR REPLACE PACKAGE        FIN_CBM_DEPOSIT_CONDITIONS AS

  /* TODO enter package declarations (types, exceptions, methods etc) here */
  PROCEDURE FIN_CBM_DEPOSIT_CONDITIONS (	inp_str      IN  VARCHAR2,
                                          out_retCode  OUT NUMBER,
                                          out_rec      OUT VARCHAR2 );

END FIN_CBM_DEPOSIT_CONDITIONS;


/


CREATE OR REPLACE PACKAGE BODY                                                                                                                                             FIN_CBM_DEPOSIT_CONDITIONS AS



-------------------------------------------------------------------------------------
--Update user - Yin Win Phyu
-- Update Date - 7-4-2017
	-- Cursor declaration
 ...
 from  (select  gstt.bal_date,
        ...
        case  when  sol.State_Code = '012' then (gstt.TOT_cash_DR_AMT + gstt.TOT_xfer_DR_AMT + gstt.TOT_clg_DR_AMT) else 0 end as Dr_Yangon,
         ...
         where    gstt.bal_date =(select  bal_date 
                                  from (select max(bal_date) as bal_date
