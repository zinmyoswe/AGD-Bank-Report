CREATE OR REPLACE PACKAGE        FIN_ISS_TRANS_WITHDRAW_LISTING AS

  /* TODO enter package declarations (types, exceptions, methods etc) here */
  PROCEDURE FIN_ISS_TRANS_WITHDRAW_LISTING(	inp_str      IN  VARCHAR2,
                                            out_retCode  OUT NUMBER,
                                            out_rec      OUT VARCHAR2 );

END FIN_ISS_TRANS_WITHDRAW_LISTING;


/


CREATE OR REPLACE PACKAGE BODY                                                                                                                                                                  FIN_ISS_TRANS_WITHDRAW_LISTING AS
/******************************************************************************
 NAME:       FIN_ISS_TRANS_WITHDRAW_LISTING
 PURPOSE:
...
--------------------------------------------------------------------------------
-- CURSOR declaration FIN_ISS_TRANS_WITHDRAW_LISTING CURSOR
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- CURSOR ExtractData
--------------------------------------------------------------------------------
...
IS
  ...
  (
      ...
      AND CHANNEL_ID = 'EFT'
      AND DEL_CHANNEL_MESG_ID = 'CWDS'
      AND NVL(FREE_TEXT3,'') IS NULL
      AND NVL(FREE_TEXT2,'') NOT IN ('10300004','00240001','00000010','00000020')
      --AND TRAN_DATE BETWEEN TO_DATE(CAST(ci_startDate AS VARCHAR(10)),'dd-MM-yyyy')
      --AND TO_DATE(CAST(ci_endDate AS VARCHAR(10)),'dd-MM-yyyy')
  ) D ON C.TRAN_ID = D.TRAN_ID AND C.SOL_ID = D.SOL_ID AND C.TRAN_DATE = D.TRAN_DATE
...
IS
  ...
  (
      ...
      (SELECT DTT.TRAN_ID, DTT.TRAN_DATE, DTT.DTH_INIT_SOL_ID AS SOL_ID
         ...
         , DTT.GL_SUB_HEAD_CODE
      FROM CUSTOM.CUSTOM_CTD_DTD_ACLI_VIEW DTT
      INNER JOIN TBAADM.RTT T ON DTT.TRAN_ID = T.TRAN_ID AND DTT.DTH_INIT_SOL_ID = T.SOL_ID
      WHERE DTT.GL_SUB_HEAD_CODE IN ('70101','70111','70121','70301','10102','60100')
      AND TO_DATE(CAST(DTT.TRAN_DATE AS VARCHAR(10)),'dd-MM-yyyy') = TO_DATE(CAST(T.TRAN_DATE AS VARCHAR(10)),'dd-MM-yyyy')
      AND T.DCC_ID = 'EFT'
      AND T.CMD = 'CWDR'
...
IS
  ...
  (
      ...
      AND DEL_CHANNEL_MESG_ID = 'CWDS'
      AND NVL(FREE_TEXT3,'') IS NULL
      AND NVL(FREE_TEXT2,'') IN ('00000010','00000020')
      --AND TRAN_DATE BETWEEN TO_DATE(CAST(ci_startDate AS VARCHAR(10)),'dd-MM-yyyy')
      --AND TO_DATE(CAST(ci_endDate AS VARCHAR(10)),'dd-MM-yyyy')
  ) D ON C.TRAN_ID = D.TRAN_ID AND C.SOL_ID = D.SOL_ID AND C.TRAN_DATE = D.TRAN_DATE
  LEFT JOIN TBAADM.CXL X ON C.TRAN_ID = X.PARENT_TRAN_ID AND C.SOL_ID = X.SERVICE_SOL_ID
