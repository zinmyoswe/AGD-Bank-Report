CREATE OR REPLACE PACKAGE FIN_POS_REVERSEISSUING_LISTING AS

  /* TODO enter package declarations (types, exceptions, methods etc) here */
  PROCEDURE FIN_POS_REVERSEISSUING_LISTING(	inp_str      IN  VARCHAR2,
                                            out_retCode  OUT NUMBER,
                                            out_rec      OUT VARCHAR2 );

END FIN_POS_REVERSEISSUING_LISTING;

/


CREATE OR REPLACE PACKAGE BODY        FIN_POS_REVERSEISSUING_LISTING AS
/******************************************************************************
 NAME:       FIN_POS_REVERSEISSUING_LISTING
 PURPOSE:
...
--------------------------------------------------------------------------------
-- CURSOR declaration FIN_POS_REVERSEISSUING_LISTING CURSOR
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- CURSOR ExtractData
--------------------------------------------------------------------------------
...
IS
  ...
  (
      ...
      --AND 
      T.DCC_ID = 'EFT'
      AND T.CMD = 'PRRR' --LIKE 'P%'
      AND REGEXP_SUBSTR(DTT.TRAN_PARTICULAR,'[^/]+',1,1) = ci_channelType--DTT.TRAN_PARTICULAR LIKE ci_channelType
  ...
  (
    ...
    AND CHANNEL_ID = 'EFT'
    AND DEL_CHANNEL_MESG_ID = 'PRRS' --LIKE 'P%'
...
CURSOR PVRExtractData (ci_tranDate VARCHAR2, ci_branchCode VARCHAR2)
IS
  SELECT D.CHANNEL_DEVICE_ID AS TERMINAL_ID, DTT.TRAN_ID AS ENTRY_NO, ' ' AS CARD_NUMBER
     , D.FORACID AS ACC_NO, G.ACCT_NAME AS ACC_NAME, C.VALUE_DATE AS DATEPART, TO_CHAR(C.PSTD_DATE, 'HH24:MI:SS') AS TIMEPART
     , REGEXP_SUBSTR(C.TRAN_PARTICULAR,'[^/]+',1,1) AS TRAN_TYPE
  ...
  (
      ...
      AND REGEXP_SUBSTR(DTT.TRAN_PARTICULAR,'[^/]+',1,1) = 'PVR'
      AND DTT.DTH_INIT_SOL_ID LIKE '%' || ci_branchCode || '%'
      AND DTT.VALUE_DATE = TO_DATE(CAST(ci_tranDate AS VARCHAR(10)),'dd-MM-yyyy'))
      PIVOT (SUM(CREDIT) AS A , SUM(DEBIT) AS B FOR (GL_SUB_HEAD_CODE)
      IN ('70301' AS A, '70101' AS B, '70111' AS C, '70121' AS D))
  ...
  INNER JOIN
  (
      SELECT G.TRAN_ID, G.TRAN_DATE, G.PSTD_DATE, G.VALUE_DATE, G.ACID, G.SOL_ID, G.TRAN_PARTICULAR, S.MESG
      FROM CUSTOM.CUSTOM_CTD_DTD_ACLI_VIEW G
      INNER JOIN TBAADM.RTT S ON G.TRAN_ID = S.TRAN_ID AND G.SOL_ID = S.SOL_ID AND G.TRAN_DATE = S.TRAN_DATE
      WHERE G.GL_SUB_HEAD_CODE IN ('70101','70111','70121')
...
CURSOR OPVRExtractData (ci_tranDate VARCHAR2, ci_branchCode VARCHAR2)
IS
  SELECT D.CHANNEL_DEVICE_ID AS TERMINAL_ID, DTT.TRAN_ID AS ENTRY_NO, ' ' AS CARD_NUMBER
     , D.FORACID AS ACC_NO, G.ACCT_NAME AS ACC_NAME, C.VALUE_DATE AS DATEPART, TO_CHAR(C.PSTD_DATE, 'HH24:MI:SS') AS TIMEPART
     , REGEXP_SUBSTR(C.TRAN_PARTICULAR,'[^/]+',1,1) AS TRAN_TYPE
  ...
  --------------------------------------------------------------------------------------------
    ...
    IF vi_channelType = 'Purchase Reversal Transaction' THEN
      vi_channelType := 'PUR';
    END IF;
    IF vi_channelType = 'Purchase Void Reversal Transaction' THEN
      vi_channelType := 'PVR';
    END IF;
    IF vi_channelType = 'Purchase Void Transaction' THEN
      vi_channelType := 'PVC';
    END IF;
    IF vi_channelType = 'PreAuth Reversal Transaction' THEN
      vi_channelType := 'PreAuthR';
    END IF;
    IF vi_channelType = 'PreAuth Void Transaction' THEN
      vi_channelType := 'PreAuthV';
    END IF;
    IF vi_channelType = 'PreAuth Void Reversal Transaction' THEN
      vi_channelType := 'PreAuthVR';
    END IF;
  ...
  IF vi_tranType = 'Local Transaction' THEN
      ...
      IF vi_channelType = 'PVR' THEN
    
        IF NOT PVRExtractData%ISOPEN THEN
        --{
          BEGIN
          --{
            OPEN PVRExtractData (vi_tranDate, vi_branchCode);
          --}
          END;
        --}
        END IF;
    
        IF PVRExtractData%ISOPEN THEN
        --{
          FETCH	PVRExtractData
  ...
  ------------------------------------------------------------------------------
          IF PVRExtractData%NOTFOUND THEN
          --{
            CLOSE PVRExtractData;
            out_retCode:= 1;
            RETURN;
          --}
          END IF;
  ...
  ------------------------------------------------------------------------------
      IF vi_channelType = 'PVR' THEN
  ------------------------------------------------------------------------------    
        IF NOT OPVRExtractData%ISOPEN THEN
        --{
          BEGIN
          --{
            OPEN OPVRExtractData (vi_tranDate, vi_branchCode);
          --}
          END;
        --}
        END IF;
    
        IF OPVRExtractData%ISOPEN THEN
        --{
          FETCH	OPVRExtractData
          INTO v_TerminalID, v_EntryNo, v_CardNumber, v_AccNo, v_AccName,
               v_DatePart, v_TimePart, v_TranType, v_Debit, v_Credit, v_ICredit;
  ------------------------------------------------------------------------------
        -- Here it is checked whether the cursor has fetched
        ...
        -- and the out ret code is made equal to 1
  ------------------------------------------------------------------------------
          IF OPVRExtractData%NOTFOUND THEN
          --{
            CLOSE OPVRExtractData;
            out_retCode:= 1;
            RETURN;
          --}
          END IF;
