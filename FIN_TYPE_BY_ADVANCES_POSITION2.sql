CREATE OR REPLACE PACKAGE FIN_TYPE_BY_ADVANCES_POSITION2 AS 

  PROCEDURE FIN_TYPE_BY_ADVANCES_POSITION2(	inp_str      IN  VARCHAR2,
			out_retCode  OUT NUMBER,
			out_rec      OUT VARCHAR2 );


END FIN_TYPE_BY_ADVANCES_POSITION2;
/


CREATE OR REPLACE PACKAGE BODY                                                                                                                               FIN_TYPE_BY_ADVANCES_POSITION2 AS
...
(select count(q.no) as no,
        ...
        q.br_open_date
from
(select (foracid) as no,
      (sanct_lim) as Overdraft_Limit_amt,
      (amount) as amount,
      case when amount > 0 and sanct_lim= 0 then sanct_lim
      when amount > 0 and (sanct_lim >amount) then (sanct_lim-amount)
      when tran_date_bal > 0 then sanct_lim else 0 end as unused_limit,
      sol_id,
      br_open_date from(
      select
      gam.foracid,
      ...
      gam.sol_id as sol_id,eab.tran_date_bal,
...
select (gam.foracid) as no,
      ...
      sum(gam.clr_bal_amt) as amount,
      (sum(gam.sanct_lim)- sum(gam.clr_bal_amt)) as unused_limit,
      gam.sol_id,
      sol.br_open_date
from tbaadm.gam gam,tbaadm.sol sol,custom.coa_mp coa
where coa.gl_sub_head_code = gam.gl_sub_head_code
...
and sol.bank_code = '116'
and coa.group_code ='A23'
AND (gam.ACCT_CLS_FLG = 'N' or gam.acct_cls_date is null)
AND gam.BANK_ID = '01'
--and gam.sanct_lim <>0
...
and gam.acid not in (SELECT EAB.ACID
                                   ...
                                   and gam.schm_type ='CAA'
                                     ...
                                     /*and ((gam.sanct_lim =0 
                                      ...
                                      AND    GAM.BANK_ID = '01'
                                      AND    GAM.ENTITY_CRE_FLG = 'Y')
group by sol.br_open_date,gam.sol_id,gam.foracid)q
--where q.amount >0
